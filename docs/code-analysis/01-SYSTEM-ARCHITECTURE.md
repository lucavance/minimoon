...
真实 Developer Tools 证据属于精确产物指纹，而不是某个源码提交的泛化证明。因此纯源码拆分即使 `.mbti` 不变，只要 MoonBit minifier 生成字节改变，也必须视为新的宿主产物重新验证。

> **English:**
>
> Real Developer Tools evidence belongs to an exact artifact fingerprint, not to a source commit in the abstract. Therefore, even a source-only split with unchanged `.mbti` requires new host validation if MoonBit minification changes generated bytes.

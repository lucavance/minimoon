...
当前测试分布在 root API、graph、renderer、runtime、tooling 和 host templates。声明数量本身不是目标；关键路径还通过 package coverage floors 和特定负向 fixture 防止只覆盖 happy path。

> **English:**
>
> Current tests span the root API, graph, renderer, runtime, tooling, and host templates. Declaration count is not the goal. Critical paths also use package coverage floors and targeted negative fixtures to prevent happy-path-only coverage.

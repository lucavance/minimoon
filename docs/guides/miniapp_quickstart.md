...
- MoonBit: `moon 0.1.20260920` with `moonc v0.10.14` or newer.
  Upgrade older installations before using core `0.2.3` or UI `0.1.1`.
- Global `vp 0.3.3`, managing Node `26.9.0` and Bun `1.4.2`.
...
vp env setup
vp env on
vp env install node@26.9.0 bun@1.4.2
moon version --all
vp --version
vp env exec --node 26.9.0 --package-manager bun@1.4.2 node --version
```
...
`devEngines.runtime` Node `26.9.0` and `devEngines.packageManager` Bun `1.4.2`;
inside either directory, `vp node --version` should report `v26.9.0` and
`vp env current` shows both selected versions. No `.node-version` is needed.
CI explicitly checks Node `24.20.0` as well as `26.9.0`.
...
The global `vp` CLI is required by Minimoon `0.2.3` build/verification tools.
...
This source documents core `0.2.3` with optional UI `0.1.1`. The commands pin
...
The `0.2.3` CLI uses `vp` for all JavaScript tool launches, with managed Bun
...
The onboarding command pins `0.2.3` so the CLI and generated dependency agree.
...
Confirm `minimoon --version` prints `0.2.3`, then follow the initialization and
...
With the `0.2.3` CLI, an ordinary build failure keeps the previous `dist/` and
...
for core `0.2.3` (UI remains the published `0.1.1`); it does not mark your app or either fixture as

...
- MoonBit: `moon 0.1.20260920` with `moonc v0.10.14` or newer.
  Upgrade older installations before using core `0.2.2` or UI `0.1.1`.
...
CI covers Node 24.20.0 and 26.9.0. Keep the dependency versions and PostCSS
override generated by `minimoon init`.

Vite+ is optional environment management, not a Minimoon package dependency.
The reviewed global CLI is `vp 0.3.3`. A Node support range does not select the
CI primary version: compare `node --version` with `vp node --version` when using
its shims. For an explicit environment without adding `.node-version`, run
`vp env exec --node 26.9.0 --package-manager bun@1.4.2 <command>`.

<a id="registry-installation-after-publication"></a>

## Registry installation

This source documents core `0.2.2` with optional UI `0.1.1`. The commands pin
...
The `0.2.2` CLI includes the App-aware helper fix and explicit package-qualified
API calls needed by the new compiler. Enabling `application` no longer requires
...
The onboarding command pins `0.2.2` so the CLI and generated dependency agree.
...
Confirm `minimoon --version` prints `0.2.2`, then follow the initialization and
...
With the `0.2.2` CLI, an ordinary build failure keeps the previous `dist/` and
...
for core `0.2.2` / UI `0.1.1`; it does not mark your app or either fixture as
